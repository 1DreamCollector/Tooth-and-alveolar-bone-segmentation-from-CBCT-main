...
parser.add_argument('--root_path', type=str, default='/data1/LJX/stage_v1/', help='Name of Experiment')
parser.add_argument('--exp', type=str,  default='NC_1st_stage_sklV2_HZ_02_(1000data_256size_intensityClip)', help='model_name')
parser.add_argument('--max_iterations', type=int,  default=60000, help='maximum epoch number to train')
parser.add_argument('--batch_size', type=int, default=4, help='batch_size per gpu')
parser.add_argument('--cnt_or_skl', type=str, default='/h5_1st_stage_v1.txt', help='train skl-net or cnt-net')
...
if __name__ == "__main__":
    ...
    db_train = toothLoader(base_dir=train_data_path,
                       split='train',cnt_or_skl =cnt_or_skl ,
    ...
    db_test = toothLoader(base_dir=train_data_path,
                       split='test',cnt_or_skl =cnt_or_skl,
                       transform = transforms.Compose([
                           #RandomCrop(patch_size),
                           #DataScale(),
                           ToTensor()
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            #用于计算牙齿物体体素上的偏移回归误差
            loss_off = reg_criterion(outputs_off[:, :, label_batch[0, :, :, :]==1], offset_skl_batch[:, :, label_batch[0, :, :, :]==1])

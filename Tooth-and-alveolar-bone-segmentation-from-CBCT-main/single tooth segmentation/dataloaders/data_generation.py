...
def rescale(image_label, w_ori, h_ori, z_ori, flag):
    ...
    if flag == 'trilinear':
        ...
        image_label = torch.from_numpy(image_label.astype(float)).cuda()
...
def read_data(data_path_img, data_patch_lab):
    ...
    spacing = src_data_vol.header['pixdim'][1:4]  # xyz三个方向上相邻体素之间的间距
    w, h, d = images.shape
    # 统一不同扫描的体素尺寸
...
def covert_h5(images, labels, skeletons, bds, data_id, data_path):
    ...
    for i_rot in range(1):
        ...
        for i in range(len(teeth_ids)):
            ...
            bin_tooth_label = morphology.remove_small_objects((labels == tooth_id), 500)  # 移除面积小于500的标记区域
            coord = np.nonzero(bin_tooth_label)  # 找到对应牙号的牙齿在三个轴上的非零区域的索引
            if coord[0].shape[0] < 500:
                continue
            meanx = int(np.mean(coord[0]))
            meany = int(np.mean(coord[1]))
            meanz = int(np.mean(coord[2]))
            # 上面三行把质心找到了, 牙齿在三个轴的非零区域的中点, 三个通道分别存放着三个维度上均值与原始索引的差
        ...
        # 在roi中随机裁剪了十个128*128*128的区域
        image_list, label_list, offset_cnt_list, offset_skl_list = random_crop(images, labels, offset, offset_skl)

        for i_file in range(len(image_list)):
            print('---save file:', i_file)
            f = h5py.File('/data1/LJX/stage_v1/h5_1st_stage_v1/' + data_path[(path_pos_2 + 1):path_pos_3] + '_' + str(
                i_file) + '.h5', 'w')
...
if __name__ == '__main__':
    ...
    with open("/data1/LJX/stage_v1/skl.txt", 'r') as f:
        skl_list = f.readlines()
    skl_list = [item.replace('\n', '') for item in skl_list]

    with open("/data1/LJX/stage_v1/bd.txt", 'r') as f:
        bd_list = f.readlines()
    bd_list = [item.replace('\n', '') for item in bd_list]

    for data_id in range(0, len(image_list)):
        print('process the data:', data_id)
        images, labels = read_data(image_list[data_id], label_list[data_id])
        skeletons = skl_list[data_id]
        skeletons = nib.load(skeletons)
        skeletons = skeletons.get_fdata()
        bds = bd_list[data_id]
        bds = nib.load(bds)
        bds = bds.get_fdata()
        covert_h5(images, labels, skeletons, bds, data_id, image_list[data_id])
...
def random_crop(image, label):
    '''这里返回5个随机裁剪的roi区域，这个区域大小是256*256*256'''
...
def covert_h5(images, labels):
    '''将随机裁剪的roi区域保存成h5文件，这是一对ct数据对应5个随机裁剪的ROI区域'''
    image_list, label_list = random_crop(images, labels)
    for file_i in range(len(image_list)):
        f = h5py.File('/data1/LJX/h5_roi/' + str(data_id+678) + '_' + str(file_i) + '_roi.h5', 'w')
